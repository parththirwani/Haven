...
function FeatureCard({
  ...
  return (
    <motion.div
      ...
      className="group relative p-6 rounded-3xl cursor-default overflow-hidden h-full"
      style={{
    ...
    >
      ...
      {/* Icon */}
      <div
        className={`relative w-11 h-11 rounded-2xl bg-gradient-to-br ${ac.gradientFrom} ${ac.gradientTo} 
                    flex items-center justify-center mb-5 ring-1 ring-white/10 
                    transition-all duration-500 group-hover:scale-110 group-hover:rotate-6`}
      >
        <Icon 
          size={18} 
          className={`${ac.text} transition-all duration-500 group-hover:scale-110`} 
          strokeWidth={1.8} 
        />
      </div>

      {/* Text */}
      <h3 className="relative text-[15px] font-medium text-white tracking-[-0.01em] mb-2 transition-colors group-hover:text-white">
        {feature.title}
      </h3>
      <p className="relative text-[13px] text-zinc-400 leading-relaxed font-light transition-colors group-hover:text-zinc-300">
        {feature.desc}
      </p>

      {/* Subtle shine effect on hover */}
      <div className="absolute -inset-x-4 -top-10 h-20 bg-gradient-to-b from-white/10 to-transparent opacity-0 group-hover:opacity-30 transition-opacity duration-700 pointer-events-none" />
    </motion.div>
  );
}
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <h2
            className="text-[2.1rem] font-light tracking-[-0.03em] text-white leading-[1.15] max-w-md"
